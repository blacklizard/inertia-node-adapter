...
import inertia from '../../lib/koaAdapter';
import flash from '../../lib/koaFlash';
...
const version = '1';
export function koaServer({ sandbox }) {
    const app = new Koa();
    ...
    app.use(flash());
    app.use(serve(path.join(__dirname, '..', '..', 'www'), { brotli: true, gzip: true }));
    app.use(inertia({
        version,
        html: getHtml,
